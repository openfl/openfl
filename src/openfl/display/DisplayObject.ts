...
import RenderEvent from "openfl/events.RenderEvent;
	...
	**/
	export class DisplayObject extends EventDispatcher implements IBitmapDrawable
	{
		...
		**/
		...
		protected static __childIterators: ObjectPool<DisplayObjectIterator> = new ObjectPool<DisplayObjectIterator>(function () { return new DisplayObjectIterator(); });
		...
		**/
			...
			switch (type)
			{
				...
				case Event.RENDER:
					if (!this.hasEventListener(type))
					{
						if (DisplayObject.__broadcastEvents.exists(type))
						{
							DisplayObject.__broadcastEvents.get(type).remove(this);
						}
					}
					break;
		...
		{
			var iterator = this.__childIterators.get();
		...
		{
			for (child in this.__childIterator(false))
			{
		...
		protected __dispatchChildren(event: Event): void
		{
			if (this.__type != null)
			{
				switch (this.__type)
				{
					case DISPLAY_OBJECT_CONTAINER:
					case MOVIE_CLIP:
						var displayObjectContainer: DisplayObjectContainer = this as DisplayObjectContainer;
