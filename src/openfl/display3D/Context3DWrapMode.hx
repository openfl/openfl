...
#if (haxe_ver >= 4.0) enum #else @:enum #end abstract Context3DWrapMode(Null<Int>)
{
	...
	**/
	...
	#if cs
	@:op(A == B) private static function equals(a:Null<Context3DWrapMode>, b:Null<Context3DWrapMode>):Bool
	{
		return NullUtils.intEquals(a, b);
	}

	@:op(A != B) private static inline function notEquals(a:Null<Context3DWrapMode>, b:Null<Context3DWrapMode>):Bool
	{
		return !equals(a, b);
	}
	#end
