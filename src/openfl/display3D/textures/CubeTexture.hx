	...
	@:noCompletion private function __uploadCompressedTextureFromByteArray (data:ByteArray, byteArrayOffset:UInt):Void {
		...
		reader.readTextures (function (side, level, gpuFormat, width, height, blockLength, bytes) {
			...
			if (alpha && gpuFormat == 2) {
				...
				gl.compressedTexImage2D (target, level, __internalFormat, width, height, 0, new UInt8Array (#if (js && html5) @:privateAccess bytes.b.buffer #else bytes #end, 0, size));
				// GLUtils.CheckGLError ();
				...
				gl.compressedTexImage2D (target, level, alphaTexture.__internalFormat, width, height, 0, new UInt8Array (#if (js && html5) @:privateAccess bytes.b.buffer #else bytes #end, 0, size));
				// GLUtils.CheckGLError ();
				...
				gl.compressedTexImage2D (target, level, __internalFormat, width, height, 0, new UInt8Array (#if (js && html5) @:privateAccess bytes.b.buffer #else bytes #end, 0, blockLength));
				// GLUtils.CheckGLError ();
