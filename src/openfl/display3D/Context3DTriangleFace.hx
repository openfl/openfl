...
@SuppressWarnings("checkstyle:FieldDocComment") #if (haxe_ver >= 4.0) enum #else @:enum #end abstract Context3DTriangleFace(Null<Int>)
{
	...
	#if cs
	@:op(A == B) private static function equals(a:Null<Context3DTriangleFace>, b:Null<Context3DTriangleFace>):Bool
	{
		return NullUtils.intEquals(a, b);
	}

	@:op(A != B) private static inline function notEquals(a:Null<Context3DTriangleFace>, b:Null<Context3DTriangleFace>):Bool
	{
		return !equals(a, b);
	}
	#end
