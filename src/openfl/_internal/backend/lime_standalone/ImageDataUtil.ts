...
class ImageDataUtil
{
	public static displaceMap(target: Image, source: Image, map: Image, mapPoint: Point, componentX: Vector3D, componentY: Vector3D, smooth: boolean): void
	{
		...
		var targetPremultiplied: boolean = target.premultiplied;
		var sourcePremultiplied: boolean = source.premultiplied;
		var mapPremultiplied: boolean = map.premultiplied;
		...
		var mapPixelX: number;
		var mapPixelY: number;
		var mapPixelA: number;
		...
		var mPointXFloor: number;
		var mPointYFloor: number;

		var disOffsetXFloor: number;
		var disOffsetYFloor: number;
	...
	// s3 = (x + 1, y + 1);
	// s4 = (x, y)
	private static bilinear(s1: RGBA, s2: RGBA, s3: RGBA, s4: RGBA, su: number, sv: number): RGBA
	{
		return lerpRGBA(lerpRGBA(s4, s2, su), lerpRGBA(s1, s3, su), sv);
	}

	private static lerpRGBA(v0: RGBA, v1: RGBA, x: number): RGBA
	{
	...
	private static lerp4f(v0: Vector3D, v1: Vector3D, x: number): Vector3D
	{
		return new Vector3D(lerp(v0.x, v1.x, x), lerp(v0.y, v1.y, x), lerp(v0.z, v1.z, x), lerp(v0.w, v1.w, x));
	}

	private static lerp(v0: number, v1: number, x: number): number
	{
	...
	public static gaussianBlur(image: Image, sourceImage: Image, sourceRect: Rectangle, destPoint: Point, blurX: number = 4, blurY: number = 4,
		quality: number = 1, strength: number = 1, color: null | number = null): Image
	{
	...
	public static getColorBoundsRect(image: Image, mask: number, color: number, findColor: boolean, format: PixelFormat): Rectangle
	{
	...
	public static multiplyAlpha(image: Image): void
	{
		var data = image.buffer.data;
	...
	public static resize(image: Image, newWidth: number, newHeight: number): void
	{
...

		{
			...
			var sourceIndex: number,
				sourceIndexX: number,
				sourceIndexY: number,
				sourceIndexXY: number,
				index: number;
			var sourceX: number, sourceY: number;
			var u: number,
				v: number,
				uRatio: number,
				vRatio: number,
	...
	public static resizeBuffer(image: Image, newWidth: number, newHeight: number): void
	{
		var buffer = image.buffer;
		var data = image.data;
		var newData = new UInt8Array(newWidth * newHeight * 4);
		var sourceIndex: number, index: number;
	...
	public static setFormat(image: Image, format: PixelFormat): void
	{
		var data = image.buffer.data;
		if (data == null) return;
	...
	public static threshold(image: Image, sourceImage: Image, sourceRect: Rectangle, destPoint: Point, operation: string, threshold: number, color: number,
		mask: number, copySource: boolean, format: PixelFormat): number
	{
	...
	public static unmultiplyAlpha(image: Image): void
	{
		var data = image.buffer.data;
		if (data == null) return;
	...
	**/
	private static #if cpp inline #end __calculateSourceOffset(sourceRect: Rectangle, destPoint: Point, destX: number, destY: number): number
	{
		var sourceX: number = destX - Std.int(destPoint.x);
		var sourceY: number = destY - Std.int(destPoint.y);
	...
	private static __getBoxesForGaussianBlur(sigma: number, n: number): Array<Float>
	{
...
{
	public x(default , null): number;
	public y(default , null): number;
	...
	public inline hasRow(y: number): boolean
	{
		return (y >= 0 && y < height);
	}
	...
	public inline row(y: number): number
	{
...
/** @hidden */ @: enum private abstract ThresholdOperation(Int) from Int to Int
{
	var NOT_EQUALS = 0;
	var EQUALS = 1;
	var LESS_THAN = 2;
