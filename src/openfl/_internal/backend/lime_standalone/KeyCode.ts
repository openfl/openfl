...
@: enum abstract KeyCode(Int) from Int to Int from UInt to UInt
{
	...
	// NUMPAD_EQUALS_AS400 = 0x40000086;
	...
	// @:from public static fromScanCode(scanCode:ScanCode):KeyCode
	// {
	// 	return KeyCode.UNKNOWN;
	// }
	// private static toScanCode(keyCode:KeyCode):ScanCode
	// {
	// 	return ScanCode.UNKNOWN;
	// }
	@: op(A > B) private static readonly gt(a: KeyCode, b: KeyCode) : boolean
	{
		return (a: number) > (b  : number);
	}

	@: op(A >= B) private static readonly gte(a: KeyCode, b: KeyCode) : boolean
	{
		return (a: number) >= (b  : number);
	}

	@: op(A < B) private static readonly lt(a: KeyCode, b: KeyCode) : boolean
	{
		return (a: number) <(b: number);
	}

	@: op(A <= B) private static readonly lte(a: KeyCode, b: KeyCode) : boolean
	{
		return (a: number) <= (b  : number);
	}

	@: op(A + B) private static readonly plus(a: KeyCode, b : number): KeyCode
	{
		return (a: number) + b;
	}
}
