...
class StackBlur
{
	private static MUL_TABLE: Array<Int> = [
	...
	private static SHG_TABLE: Array<Int> = [
	...
	public static blur(dest: Image, source: Image, sourceRect: Rectangle, destPoint: Point, blurX: number, blurY: number, quality: number)
	{
	...
	private static __stackBlurCanvasRGBA(image: Image, width: number, height: number, blurX: number, blurY: number, quality: number)
	{
		...
		var px = image.data;
		var x: number, y: number, i: number, p: number, yp: number, yi: number, yw: number;
		var r: number, g: number, b: number, a: number, pr: number, pg: number, pb: number, pa: number;
		var f: number;
