...
@: access(openfl._internal.renderer.context3D.Context3DRenderer)
@: access(openfl._internal.renderer.dom.DOMRenderer)
...
{
	private static gameInputDevices: Map<Gamepad, GameInputDevice> = new Map();

	private macKeyboard: boolean;
	...
	private convertKeyCode(key: KeyCode): number
	{
		return switch (key)
		{
	...
	private getCharCode(key: number, shift: boolean = false): number
	{
		if (!shift)
		{
			switch (key)
	...
	private getGameInputDevice(gamepad: Gamepad): GameInputDevice
	{
	...
	private getKeyLocation(key: KeyCode): KeyLocation
	{
		return switch (key)
		{
	...
	public registerLimeModule(application: Application): void
	{
	...
	public unregisterLimeModule(application: Application): void
	{
	...
	// Event Handlers
	private application_onCreateWindow(window: Window): void
	{
	...
	private application_onExit(code: number): void
	{
		if (parent.limeWindow != null)
		{
	...
	private application_onUpdate(deltaTime: number): void
	{
		parent.__deltaTime = deltaTime;
		parent.__dispatchPendingMouseEvent();
	}

	private gamepad_onAxisMove(gamepad: Gamepad, axis: GamepadAxis, value: number): void
	{
	...
	private gamepad_onButtonDown(gamepad: Gamepad, button: GamepadButton): void
	{
		var device = getGameInputDevice(gamepad);
		if (device == null) return;
	...
	private gamepad_onButtonUp(gamepad: Gamepad, button: GamepadButton): void
	{
		var device = getGameInputDevice(gamepad);
		if (device == null) return;
	...
	private gamepad_onConnect(gamepad: Gamepad): void
	{
		var device = getGameInputDevice(gamepad);
		if (device == null) return;
	...
	private gamepad_onDisconnect(gamepad: Gamepad): void
	{
	...
	private touch_onCancel(touch: Touch): void
	{
	...
	private touch_onMove(touch: Touch): void
	{
	...
	private touch_onEnd(touch: Touch): void
	{
		var oldPrimaryTouch = primaryTouch;
		if (primaryTouch == touch)
		{
	...
	private touch_onStart(touch: Touch): void
	{
	...
	private window_onActivate(window: Window): void
	{
	...
	private window_onClose(window: Window): void
	{
	...
	private window_onCreate(window: Window): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onDeactivate(window: Window): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onDropFile(window: Window, file: string): void { }

	private window_onEnter(window: Window): void
	{
		// if (parent.limeWindow == null || parent.limeWindow != window) return;
	}

	private window_onExpose(window: Window): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onFocusIn(window: Window): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onFocusOut(window: Window): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onFullscreen(window: Window): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onKeyDown(window: Window, keyCode: KeyCode, modifier: KeyModifier): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onKeyUp(window: Window, keyCode: KeyCode, modifier: KeyModifier): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onLeave(window: Window): void
	{
	...
	private window_onMinimize(window: Window): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onMouseDown(window: Window, x: number, y: number, button: number): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onMouseMove(window: Window, x: number, y: number): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onMouseMoveRelative(window: Window, x: number, y: number): void
	{
		// if (parent.limeWindow == null || parent.limeWindow != window) return;
	}

	private window_onMouseUp(window: Window, x: number, y: number, button: number): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onMouseWheel(window: Window, deltaX: number, deltaY: number, deltaMode: MouseWheelMode): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onMove(window: Window, x: number, y: number): void
	{
		// if (parent.limeWindow == null || parent.limeWindow != window) return;
	}

	private window_onRender(context: RenderContext): void
	{
	...
	private window_onRenderContextLost(): void
	{
	...
	private window_onRenderContextRestored(context: RenderContext): void
	{
		createRenderer();
	...
	private window_onResize(window: Window, width: number, height: number): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onRestore(window: Window): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
	...
	private window_onTextEdit(window: Window, text: string, start: number, length: number): void
	{
		// if (parent.limeWindow == null || parent.limeWindow != window) return;
	}

	private window_onTextInput(window: Window, text: string): void
	{
		if (parent.limeWindow == null || parent.limeWindow != window) return;
