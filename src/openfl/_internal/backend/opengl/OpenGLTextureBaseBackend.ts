...
class OpenGLTextureBaseBackend
{
	private static glCompressedFormats: Map<Int, Int>;
	private static glCompressedFormatsAlpha: Map<Int, Int>;
	private static glTextureFormat: number;
	private static glTextureInternalFormat: number;

	private alphaTexture: TextureBase;
	private baseParent: TextureBase;
	// private compressedMemoryUsage:Int;
	// private context:Context3D;
	private contextBackend: OpenGLContext3DBackend;
	private gl: WebGLRenderingContext;
	private glDepthRenderbuffer: GLRenderbuffer;
	private glFormat: number;
	private glFramebuffer: GLFramebuffer;
	private glStencilRenderbuffer: GLRenderbuffer;
	private glInternalFormat: number;
	private glTextureID: GLTexture;
	private glTextureTarget: number;
	...
	private getGLFramebuffer(enableDepthAndStencil: boolean, antiAlias: number, surfaceSelector: number): GLFramebuffer
	{
		if (glFramebuffer == null)
		{
		...
		if (enableDepthAndStencil && glDepthRenderbuffer == null)
		{
			...
			if (OpenGLContext3DBackend.glDepthStencil != 0)
			{
				...
				gl.renderbufferStorage(GL.RENDERBUFFER, OpenGLContext3DBackend.glDepthStencil, baseParent.__width, baseParent.__height);
...
private getImage(bitmapData: BitmapData): Image
{
		#if lime
...
private getTexture(): GLTexture
{
	return glTextureID;
}

private setSamplerState(state: SamplerState) : boolean
{
	...
	#if(lime || openfl_html5)
private uploadFromImage(image: Image): void
	{
