...
{
	public static glDepthStencil: number = -1;
	public static glMaxTextureMaxAnisotropy: number = -1;
	public static glMaxViewportDims: number = -1;
	public static glMemoryCurrentAvailable: number = -1;
	public static glMemoryTotalAvailable: number = -1;
	public static glTextureMaxAnisotropy: number = -1;

	private static driverInfo: string;

	#if lime
	public limeContext: RenderContext;
	#end
	...
	public setGLSLProgramConstantsFromMatrix(programType: Context3DProgramType, firstRegister: number, matrix: Matrix3D, transposedMatrix: boolean = false): void
	{
		flushGLProgram();
	...
	private bindGLArrayBuffer(buffer: GLBuffer): void
	{
	...
	private bindGLElementArrayBuffer(buffer: GLBuffer): void
	{
	...
	private bindGLFramebuffer(framebuffer: GLFramebuffer): void
	{
	...
	private bindGLTexture2D(texture: GLTexture): void
	{
	...
	private bindGLTextureCubeMap(texture: GLTexture): void
	{
		// TODO: Need to consider activeTexture ID
	...
	private _drawTriangles(firstIndex: number = 0, count: number): void
	{
		#if!openfl_disable_display_render
		if (parent.__state.renderToTexture == null)
		{
	...
	private inline flushGLColor(): void
	{
	...
	private flushGLFramebuffer(): void
	{
		if (parent.__state.renderToTexture != null)
		{
	...
	private getGLBlend(blendFactor: Context3DBlendFactor): number
	{
		switch (blendFactor)
		{
	...
	private getGLCompareMode(mode: Context3DCompareMode): number
	{
		return switch (mode)
		{
	...
	private getGLStencilAction(action: Context3DStencilAction): number
	{
	...
	private getGLTriangleFace(face: Context3DTriangleFace): number
	{
		return switch (face)
		{
			case FRONT: GL.FRONT;
	...
	public resetGLBlendEquation(): void
	{
		setGLBlendEquation(GL.FUNC_ADD);
	}

	private setGLBlend(enable: boolean): void
	{
	...
	private setGLBlendEquation(value: number): void
	{
	...
	private setGLCullFace(enable: boolean): void
	{
	...
	private setGLDepthTest(enable: boolean): void
	{
	...
	private setGLFrontFace(counterClockWise: boolean): void
	{
	...
	private setGLScissorTest(enable: boolean): void
	{
	...
	private setGLStencilTest(enable: boolean): void
	{
