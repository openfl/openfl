...
class HTML5StageBackend
{
	...
	private convertBrowserKeyCode(keyCode: number): KeyCode
	{
	...
	// Event Handlers
	private window_onBeforeUnload(event: JSEvent): void { }

	private window_onBlur(event: JSEvent): void { }

	private window_onDeviceMotion(event: JSDeviceMotionEvent): void { }

	private window_onFocus(event: JSEvent): void { }

	private window_onKeyDown(event: JSKeyboardEvent): void
	{
	...
	private window_onKeyUp(event: JSKeyboardEvent): void
	{
		// TODO: Remove intermediate KeyCode/KeyModifier type?
		var keyCode = convertBrowserKeyCode(event.keyCode != null ? event.keyCode : event.which);
		var modifier: KeyModifier = (event.shiftKey ? (KeyModifier.SHIFT) : 0) | (event.ctrlKey ? (KeyModifier.CTRL) : 0) | (event.altKey ? (KeyModifier.ALT) : 0) | (event.metaKey ? (KeyModifier.META) : 0);
	...
	private window_onRequestAnimationFrame(?__): void
	{
	...
	private window_onResize(event: JSEvent): void { }
}
#end
