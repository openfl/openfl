	...
	public static function render (tilemap:Tilemap, renderSession:RenderSession):Void {
		...
		for (i in 0...(drawCount + 1)) {
			...
			if (flush) {
				...
				var image = @:privateAccess cacheBitmapData.__khaImage;
				...
				g.drawIndexedVertices (lastIndex * 6, (i - lastIndex) * 6);
			...
			if (shader != cacheShader) {
				...
				g.setMatrix (uMatrix, KhaUtils.convertMatrix (renderer.getMatrix (tilemap.__renderTransform)));
			...
			if (i == drawCount && tileset.__bitmapData != null) {
				...
				var image = @:privateAccess tileset.__bitmapData.__khaImage;
				var uImage0 = pipeline.getTextureUnit ("uImage0");
				g.setTexture (uImage0, image);
