...
{
	private static __alphaValue: Array<Float> = [1];
	private static __childRendererPool: ObjectPool<Context3DRenderer>;
	private static __colorMultipliersValue: Array<Float> = [0, 0, 0, 0];
	private static __colorOffsetsValue: Array<Float> = [0, 0, 0, 0];
	private static __defaultColorMultipliersValue: Array<Float> = [1, 1, 1, 1];
	private static __emptyColorValue: Array<Float> = [0, 0, 0, 0];
	private static __emptyAlphaValue: Array<Float> = [1];
	private static __hasColorTransformValue: Array<Bool> = [false];
	private static __scissorRectangle: Rectangle = new Rectangle();
	private static __textureSizeValue: Array<Float> = [0, 0];

	public batcher: BatchRenderer = null;
	public context3D: Context3D;

	private __alphaMaskShader: Context3DAlphaMaskShader;
	private __clipRects: Array<Rectangle>;
	#if lime
	private __limeContext: RenderContext;
	#end
	private __currentDisplayShader: Shader;
	private __currentGraphicsShader: Shader;
	private __currentRenderTarget: BitmapData;
	private __currentShader: Shader;
	private __currentShaderBuffer: ShaderBuffer;
	private __defaultDisplayShader: DisplayObjectShader;
	private __defaultGraphicsShader: GraphicsShader;
	private __defaultRenderTarget: BitmapData;
	private __defaultShader: Shader;
	private __displayHeight: number;
	private __displayWidth: number;
	private __flipped: boolean;
	private __getMatrixHelperMatrix: Matrix = new Matrix();
	private __gl: WebGLRenderContext;
	private __height: number;
	private __maskShader: Context3DMaskShader;
	private __matrix: #if (!lime && openfl_html5) Matrix3D #else Matrix4 #end;
	private __maskObjects: Array<DisplayObject>;
	private __numClipRects: number;
	private __offsetX: number;
	private __offsetY: number;
	private __projection:#if(!lime && openfl_html5) Matrix3D #else Matrix4 #end;
	private __projectionFlipped:#if(!lime && openfl_html5) Matrix3D #else Matrix4 #end;
	private __scrollRectMasks: ObjectPool<Shape>;
	private __softwareRenderer: DisplayObjectRenderer;
	private __stencilReference: number;
	private __tempColorTransform: ColorTransform;
	private __tempRect: Rectangle;
	private __updatedStencil: boolean;
	private __upscaled: boolean;
	private __values: Array<Float>;
	private __width: number;

private new (context: Context3D, defaultRenderTarget: BitmapData = null)
{
	super(context);
...
publicapplyAlpha(alpha : number): void
	{
...
publicapplyColorTransform(colorTransform: ColorTransform): void
	{
...
publicapplyHasColorTransform(enabled : boolean): void
	{
...
publicapplyMatrix(matrix: Array<Float>): void
	{
		if(__currentShaderBuffer != null)
{
...
publicgetMatrix(transform: Matrix): #if(!lime && openfl_html5) Matrix3D #else Matrix4 #end
{
	if (__gl != null)
	{
...
publicsetShader(shader: Shader): void
	{
...
publicuseColorTransformArray(): void
	{
		if(__currentShader != null)
{
...
private __copyShader(other: Context3DRenderer): void
	{
...
private __fillRect(bitmapData: BitmapData, rect: Rectangle, color : number): void
	{
...
private __getDisplayTransformTempMatrix(transform: Matrix, pixelSnapping: PixelSnapping): Matrix
{
...
private __getMatrix(transform: Matrix, pixelSnapping: PixelSnapping): Array < Float >
{
...
private __init(context: Context3D, defaultRenderTarget: BitmapData): void
	{
		context3D = context;
		#if lime
...
private __initShader(shader: Shader): Shader
{
	if (shader != null)
	{
...
private __initDisplayShader(shader: Shader): Shader
{
	if (shader != null)
	{
		// TODO: Change of GL context?
...
private __initGraphicsShader(shader: Shader): Shader
{
	if (shader != null)
	{
		// TODO: Change of GL context?
...
private __initShaderBuffer(shaderBuffer: ShaderBuffer): Shader
{
...
private __renderBitmapData(bitmapData: BitmapData): void
	{
		__setBlendMode(NORMAL);
...
private __renderFilterPass(source: BitmapData, shader: Shader, smooth : boolean, clear : boolean = true): void
	{
...
private __renderMask(mask: DisplayObject): void
	{
		if(mask != null)
{
	switch (mask.__type)
...
private __resumeClipAndMask(childRenderer: Context3DRenderer): void
	{
		if(__stencilReference > 0)
{
	context3D.setStencilActions(FRONT_AND_BACK, EQUAL, KEEP, KEEP, KEEP);
...
private __scissorRect(clipRect: Rectangle = null): void
	{
		#if!disable_batcher
	batcher.flush();
		#end
...
private __setRenderTarget(renderTarget: BitmapData): void
	{
...
private __setShaderBuffer(shaderBuffer: ShaderBuffer): void
	{
		setShader(shaderBuffer.shader);
		__currentShaderBuffer = shaderBuffer;
	}

private __shouldCacheHardware(object: DisplayObject, value: Null<Bool>): Null < Bool >
	{
...
switch (object.__type)
{
	...
	private inline __shouldSnapToPixel(bitmap: Bitmap) : boolean
{
...
private __suspendClipAndMask(): void
	{
		if(__stencilReference > 0)
{
	context3D.setStencilActions();
...
private __updateShaderBuffer(bufferOffset : number): void
	{
