...
{
	private static readonly VERTEX_BUFFER_STRIDE: number = 5;

	private static __textureSizeValue: Array<Float> = [0, 0.];

	private static getIndexBuffer(video: Video, context: Context3D): IndexBuffer3D
	{
		#if(lime && openfl_gl)
	...
	private static getTexture(video: Video, context: Context3D): RectangleTexture
	{
		#if openfl_html5
	...
	private static getVertexBuffer(video: Video, context: Context3D): VertexBuffer3D
	{
		#if(lime && openfl_gl)
