...
class DOMTextField
{
	private static __regexColor: EReg = ~/color=("#([^"]+)"|'#([^']+)')/i;
	private static __regexFace: EReg = ~/face=("([^"]+)"|'([^']+)')/i;
	private static __regexFont: EReg = ~/<font ([^>]+)>/gi;
	private static __regexCloseFont: EReg = new EReg("</font>", "gi");
	private static __regexSize: EReg = ~/size=("([^"]+)"|'([^']+)')/i;
	...
	public static readonly render(textField: TextField, renderer: DOMRenderer): void
	{
		#if openfl_html5
		var textEngine = textField.__textEngine;
			...
			if (textField.__dirty || textField.__renderTransformChanged || textField.__div == null)
			{
				if (textEngine.text != "" || textEngine.background || textEngine.border || textEngine.type == INPUT)
				{
					if (textField.__div == null)
					{
						...
						textField.__div.addEventListener("input", (event)
					{
