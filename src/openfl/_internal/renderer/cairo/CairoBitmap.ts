...
class CairoBitmap
{
	public static readonly render(bitmap: Bitmap, renderer: CairoRenderer): void
	{
		if (!bitmap.__renderable) return;

		var alpha = renderer.__getAlpha(bitmap.__worldAlpha);
		// colorTransform = renderer.__getColorTransform (bitmap.__worldColorTransform);
		// blendMode = renderer.__getBlendMode (bitmap.__worldBlendMode);
