...
class CairoShape
{
	private static sourceTransform: Matrix3 = new Matrix3();

	public static render(shape: DisplayObject, renderer: CairoRenderer): void
	{
		...
		if (graphics != null)
		{
			...
			if (cairo != null && graphics.__visible && width >= 1 && height >= 1)
			{
				...
				if (scale9Grid != null && transform.b == 0 && transform.c == 0)
				{
					...
					function drawImage(sx: number, sy: number, sWidth: number, sHeight: number, dx: number, dy: number, dWidth: number, dHeight: number): void
					{
