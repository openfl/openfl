...
{
	public static render(textField: TextField, renderer: CairoRenderer, transform: Matrix): void
	{
		...
		if (cairo != null)
		{
			// surface:CairoImageSurface = cast cairo.target;
		...
		if (textEngine.text != null && textEngine.text != "")
		{
			...
			for (group in textEngine.layoutGroups)
			{
				...
				if (font != null && group.format.size != null)
				{
					...
					var glyphs = [];
					var x: number = group.offsetX + scrollX - bounds.x;
					var y: number = group.offsetY + group.ascent + scrollY - bounds.y;
