...
class CanvasShape
{
	public static readonly render(shape: DisplayObject, renderer: CanvasRenderer): void
	{
			...
			if (canvas != null && graphics.__visible && width >= 1 && height >= 1)
			{
				...
				if (scrollRect == null || (scrollRect.width > 0 && scrollRect.height > 0))
				{
					...
					if (scale9Grid != null && transform.b == 0 && transform.c == 0)
					{
						...
						var left: number = Std.int(Math.max(1, Math.round(scale9Grid.x * scaleX)));
						var top = Math.round(scale9Grid.y * scaleY);
						var right: number = Std.int(Math.max(1, Math.round((bounds.right - scale9Grid.right) * scaleX)));
