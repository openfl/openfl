...
class TextEngine
{
	...
	protected __textLayout: TextLayout;
	#end
	protected __texture: GLTexture;
	// protected __tileData:Map<Tilesheet, Array<Float>>;
	// protected __tileDataLength:Map<Tilesheet, Int>;
	// protected __tilesheets:Map<Tilesheet, Bool>;
	private __useIntAdvances: Null<Bool>;
...
inline
		#end
function getPositions(text: string, startIndex: number, endIndex: number): Array<#if(openfl_html5 || !lime) Float #else GlyphPosition #end >
{
	// TODO: optimize
...
return [];
		...
		#if!js inline #end getPositionsWidth(positions:#if(openfl_html5 || !lime) Array < Float > #else Array < GlyphPosition > #end) : number

{
	...
	return width;
		...
		#if!js inline #end getTextWidth(text: string) : number

{
	...
	return 0;
		...
		#if!js inline #end getBaseX() : number

{
	...
	return GUTTER + leftMargin + blockIndent + (firstLineOfParagraph ? indent : 0);
		...
		#if!js inline #end getWrapWidth() : number

{
	...
	return width - GUTTER - rightMargin - getBaseX();
		...
		#if!js inline #end nextLayoutGroup(startIndex, endIndex): void
	...
	else
{
	...
	layoutGroup.endIndex = endIndex;
		...
		#if!js inline #end setLineMetrics(): void
...
if (ascent > maxAscent)
{
	maxAscent = ascent;
		...
		#if!js inline #end setParagraphMetrics(): void
...
if (currentFormat.tabStops != null)
{
	// TODO, may not actually belong in paragraph metrics
		...
		#if!js inline #end nextFormatRange() : boolean

{
	...
	return false;
		...
		#if!js inline #end setFormattedPositions(startIndex : number, endIndex : number)

{
	...
	else
	{
		...
		#if!js inline #end placeFormattedText(endIndex : number): void
...
textIndex = endIndex;
		...
		#if!js inline #end alignBaseline(): void
...
firstLineOfParagraph = false; // TODO: need to thoroughly test this
		...
		#if!js inline #end breakLongWords(endIndex : number): void
	...
	// positions only contains the final unbroken line at the end
		...
		#if!js inline #end placeText(endIndex : number): void
