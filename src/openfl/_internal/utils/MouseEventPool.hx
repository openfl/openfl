...
class MouseEventPool {
	...
	public dynamic function create(type:String, stageX:Float, stageY:Float, local:Point, target:InteractiveObject, delta:Int = 0):MouseEvent {
		var event = new MouseEvent(type, true, false, null != local ? local.x : 0, null != local ? local.y : 0, null, MouseEvent.__ctrlKey, MouseEvent.__altKey, MouseEvent.__shiftKey, MouseEvent.__buttonDown, delta, MouseEvent.__commandKey);
		event.stageX = stageX;
		event.stageY = stageY;
		event.target = target;
