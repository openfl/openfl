...
class AnimateTimeline extends Timeline
{
	...
	@:noCompletion private var __frameScripts2:Map<Int, Void->Void>;
	// @:noCompletion private var 218Int;
	...
	private override function __addFrameScript(index:Int, method:Void->Void):Void
	{
		if (index < 0) return;
		var frame = index + 1;
		...
		if (method != null)
		{
			if (__frameScripts2 == null)
			{
				__frameScripts2 = new Map();
			}

			__frameScripts2.set(frame, method);
		}
		else if (__frameScripts2 != null)
		{
			__frameScripts2.remove(frame);
		}
	}

	@:noCompletion private override function __attachMovieClip(movieClip:MovieClip):Void
	{
		__movieClip = movieClip;
		init();
		scenes = [new Scene("", __currentLabels, __totalFrames)];
	...
	**/
	private override function __gotoAndPlay(frame:#if (haxe_ver >= "3.4.2") Any #else Dynamic #end, scene:String = null):Void
	{
		__play();
	...
	**/
	private override function __gotoAndStop(frame:#if (haxe_ver >= "3.4.2") Any #else Dynamic #end, scene:String = null):Void
	{
		__stop();
		__goto(__resolveFrameReference(frame));
	}
	...
	private function init():Void
	{
		...
		for (i in 0...__symbol.frames.length)
		{
			...
			if (frameData.script != null)
			{
				...
				__frameScripts2.set(frame, function() { frameData.script(__movieClip); });
			}
			else if (frameData.scriptSource != null)
			{
				...
				try
				{
					...
					interp.variables.set("this", __movieClip);
					...
					__frameScripts2.set(frame, script);
					#elseif js
					...
					__frameScripts2.set(frame, wrapper);
					#end
				}
				catch (e:Dynamic)
				{
	...
	**/
	private override function __nextFrame():Void
	{
		__stop();
	...
	**/
	private override function __play():Void
	{
		if (__symbol == null || __isPlaying || __totalFrames < 2) return;
	...
	**/
	private override function __prevFrame():Void
	{
		__stop();
		__goto(__currentFrame - 1);
	}
	...
	**/
	private override function __stop():Void
	{
		__isPlaying = false;
	}
	...
	@:noCompletion private override function __updateFrameScript(deltaTime:Int):Void
	{
		if (__symbol != null && __isPlaying)
		{
	...
	@:noCompletion private override function __updateSymbol(targetFrame:Int):Void
	{
	...
	@:noCompletion private override function __evaluateFrameScripts(advanceToFrame:Int):Bool
	{
		for (frame in __currentFrame...advanceToFrame + 1)
		{
			...
			if (__frameScripts2.exists(frame))
			{
				__updateSymbol(frame);
				var script = __frameScripts2.get(frame);
				script();
	...
	@:noCompletion private override function __getNextFrame(deltaTime:Int):Int
	{
		var nextFrame:Int = 0;
	...
	@:noCompletion private override function __goto(frame:Int):Void
	{
	...
	@:noCompletion private override function __resolveFrameReference(frame:#if (haxe_ver >= "3.4.2") Any #else Dynamic #end):Int
	{
		if (Std.is(frame, Int))
		{
			return cast frame;
	...
	@:noCompletion private override function __updateFrameLabel():Void
	{
