...
export default class Clipboard
{
	...
	private __text: Object;
	private __textArea: HTMLTextAreaElement;
	...
	**/
	public clear(): void
	{
		this.setData(ClipboardFormats.TEXT_FORMAT, null);
	}
	...
	**/
	public getData(format: ClipboardFormats, transferMode: ClipboardTransferMode = null): Object
	{
		...
		{
			transferMode = ClipboardTransferMode.ORIGINAL_PREFERRED;
		}
	...
	**/
	public setData(format: ClipboardFormats, data: Object, serializable: boolean = true): boolean
	{
		...
		switch (format)
		{
			...
			case ClipboardFormats.RICH_TEXT_FORMAT:
			case ClipboardFormats.TEXT_FORMAT:
				this.__text = data as string;

				this.__textArea.value = data as string;
	...
	/**
		...
		**/
	public get formats(): Array<ClipboardFormats>
	{
		let _formats: Array<ClipboardFormats>;
		_formats = [];
		if (this.hasFormat(ClipboardFormats.HTML_FORMAT)) _formats.push(ClipboardFormats.HTML_FORMAT);
		if (this.hasFormat(ClipboardFormats.RICH_TEXT_FORMAT)) _formats.push(ClipboardFormats.RICH_TEXT_FORMAT);
		if (this.hasFormat(ClipboardFormats.TEXT_FORMAT)) _formats.push(ClipboardFormats.TEXT_FORMAT);
		return _formats;
	}
