...
class BitmapData implements IBitmapDrawable {
	...
	public function threshold (sourceBitmapData:BitmapData, sourceRect:Rectangle, destPoint:Point, operation:String, threshold:Int, color:Int = 0x00000000, mask:Int = 0xFFFFFFFF, copySource:Bool = false):Int {
		...
		var limeRect = new lime.math.Rectangle(sourceRect.x, sourceRect.y, sourceRect.width, sourceRect.height);
		var limePt   = new Vector2(destPoint.x, destPoint.y);
		return image.threshold(sourceBitmapData.image, limeRect, limePt, operation, threshold, color, mask, copySource);
	...
	public function __drawConsole (source:IBitmapDrawable, matrix:Matrix, colorTransform:ColorTransform, blendMode:BlendMode, clipRect:Rectangle, smoothing:Bool):Void {
