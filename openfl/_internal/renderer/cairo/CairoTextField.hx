...
class CairoTextField {
	...
	public static function render (textField:TextField, renderSession:RenderSession) {
		...
		if (textEngine.text != null && textEngine.text != "") {
			...
			var range;
			var offsetX = 2.0;
			var offsetY = 2.0 + textEngine.lineAscents[0] + textEngine.lineDescents[0];
				...
				while (!done) {
					...
					if (range.end >= nextLineBreak) {
						...
						if (textEngine.lineAscents.length > lineBreakIndex + 1) {
							...
							offsetY += textEngine.lineLeadings[lineBreakIndex] + textEngine.lineAscents[lineBreakIndex + 1] + textEngine.lineDescents[lineBreakIndex + 1];
