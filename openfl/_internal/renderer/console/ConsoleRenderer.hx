...
class ConsoleRenderer extends AbstractRenderer {
	...
	// TODO(james4k): move to a TextRenderer class
	private var bitmapData = new Map<Font, Map<Int, BitmapData>> ();
	private var glyphs = new Map<Font, Map<Int, Map<Glyph, Image>>> ();
	private var tilesheets = new Map<BitmapData, Tilesheet> ();
	private var tileIDs = new Map<BitmapData, Map<Glyph, Int>> ();
	...
	private function updateTextField (tf:TextField) {

		if (!tf.__dirty) {
			return;
		}

		if (((tf.__text == null || tf.__text == "") && !tf.background && !tf.border)
			|| (tf.width <= 0 || tf.height <= 0)
		) {

			tf.__tilesheets = null;
			tf.__tileData = null;
			tf.__dirty = false;

			if (tf.autoSize == TextFieldAutoSize.LEFT) {
				tf.__width = 4;
			}
		...
		tf.__tilesheets = new Array ();
		tf.__tileData = new Array ();
		...
		var measurements = tf.__measureText ();
		var textWidth = 0.0;
		...
		if (tf.__graphics == null) {
			tf.__graphics = new Graphics ();
		}

		if (tf.autoSize == TextFieldAutoSize.LEFT) {
			tf.__width = textWidth + 4;
		}

		if (tf.__ranges == null) {

			renderTextFieldTiles (tf, text, tf.__textFormat, 0, textWidth);

		} else {

			var offsetX = 0.0;

			for (i in 0...tf.__ranges.length) {

				var range = tf.__ranges[i];
				
				renderTextFieldTiles (tf, text.substring (range.start, range.end), range.format, offsetX, textWidth);
		...
		var graphics = tf.__graphics;
		graphics.clear ();

		if (tf.border || tf.background) {

			if (tf.border) {
				graphics.lineStyle (1, tf.borderColor);
			}

			if (tf.background) {
				graphics.beginFill (tf.backgroundColor);
			}

			graphics.drawRect (0.5, 0.5, tf.__width - 1, tf.__height - 1);

		}

		for (i in 0...tf.__tilesheets.length) {
			
			graphics.drawTiles (tf.__tilesheets[i], tf.__tileData[i], true, Tilesheet.TILE_RGB);
	...
	private function renderTextFieldTiles (tf:TextField, text:String, format:TextFormat, offsetX:Float, textWidth:Float):Void {

		var font = tf.__getFontInstance (format);
		...
		if (!tilesheets.exists (bitmapData)) {
			...
			var sheet = new Tilesheet (bitmapData);
			var id = new Map<Int, Int> ();
			...
			for (key in images.keys ()) {
				...
				var index = sheet.addTileRect (rect);

				id.set (key, index);
			}

			tileIDs.set (bitmapData, id);
			tilesheets.set (bitmapData, sheet);
		...
		if (tf.__tilesheets.length == 0 || tf.__tilesheets[tf.__tilesheets.length - 1] != tilesheet) {

			tf.__tilesheets.push (tilesheet);
			tf.__tileData.push (new Array ());

		}

		var tileData = tf.__tileData[tf.__tileData.length - 1];
		var lines = text.split ("\n");

		if (tf.__textLayout == null) {
			tf.__textLayout == new TextLayout ();
		}

		var textLayout = tf.__textLayout;
		...
		// TODO(james4k): multiline breaks if not LEFT aligned, but is the same
		// for GL renderer at present
		if (format.align == TextFormatAlign.RIGHT) {
			x += tf.__width - textWidth;
		} else if (format.align == TextFormatAlign.CENTER) {
			x += (tf.__width - textWidth) / 2;
		}
		...
		for (line in lines) {
			...
			for (p in textLayout.positions) {

				var image = images.get (p.glyph);

				if (image != null) {

					tileData.push (x + p.offset.x + image.x);
					tileData.push (y + p.offset.y - image.y);
					tileData.push (tileID.get (p.glyph));
					tileData.push (r);
					tileData.push (g);
					tileData.push (b);
