...
class ConvolutionFilter extends BitmapFilter {
	...
	public override function equals(filter:BitmapFilter) {
		if ( Std.is(filter, ConvolutionShader) ) {
			...
			return this.alpha == otherFilter.alpha && this.bias == otherFilter.bias && this.clamp == otherFilter.clamp
				&& this.color == otherFilter.color && this.divisor == otherFilter.divisor
				&& this.matrixX == otherFilter.matrixX && this.matrixY == otherFilter.matrixY && this.preserveAlpha && otherFilter.preserveAlpha;
		}
		return false;
	}
