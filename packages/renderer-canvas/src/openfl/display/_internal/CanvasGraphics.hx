...
class CanvasGraphics
{
	...
	public static function createBitmapFill(bitmap:BitmapData, bitmapRepeat:Bool, smooth:Bool):#if openfl_html5 CanvasPattern #else Dynamic #end
	{
		...
		setSmoothing(smooth);
		return context.createPattern((bitmap._ : _BitmapData).__getElement(), bitmapRepeat ? "repeat" : "no-repeat");
		#else
		return null;
		#end
	}
	...
	public static function createTempPatternCanvas(bitmap:BitmapData, repeat:Bool, width:Int, height:Int):#if openfl_html5 CanvasElement #else Void #end
	{
		...
		context.fillStyle = context.createPattern((bitmap._ : _BitmapData).__getElement(), repeat ? "repeat" : "no-repeat");
		context.beginPath();
		context.moveTo(0, 0);
	...
	public static function hitTest(graphics:Graphics, x:Float, y:Float):Bool
	{
		...
		if ((graphics._ : _Graphics).__commands.length == 0 || bounds == null || bounds.width <= 0 || bounds.height <= 0)
		{
			return false;
		}
		else
		{
			...
			var cacheCanvas = (graphics._ : _Graphics).__renderData.canvas;
			var cacheContext = (graphics._ : _Graphics).__renderData.context;
			(graphics._ : _Graphics).__renderData.canvas = hitTestCanvas;
			(graphics._ : _Graphics).__renderData.context = hitTestContext;
			...
			for (type in (graphics._ : _Graphics).__commands.types)
			{
				switch (type)
				{
					...
					case END_FILL:
						...
						{
							data.destroy();
							(graphics._ : _Graphics).__renderData.canvas = cacheCanvas;
							(graphics._ : _Graphics).__renderData.context = cacheContext;
							return true;
						}
			...
			switch (type)
			{
				...
				case DRAW_QUADS:
					...
					for (i in 0...length)
					{
						...
						if (bitmapFill != null)
						{
							context.drawImage((bitmapFill._ : _BitmapData).__getElement(), tileRect.x, tileRect.y, tileRect.width, tileRect.height, 0, 0,
				...
				case DRAW_RECT:
					...
					if (bitmapFill != null && !hitTesting)
					{
						...
						if (canOptimizeMatrix && st >= 0 && sl >= 0 && sr <= bitmapFill.width && sb <= bitmapFill.height)
						{
							...
							if (!hitTesting) context.drawImage((bitmapFill._ : _BitmapData).__getElement(), sl, st, sr - sl, sb - st, c.x - offsetX,
								c.y - offsetY, c.width, c.height);
						}
					}
	...
	public static function render(graphics:Graphics, renderer:CanvasRenderer):Void
	{
		...
		if ((graphics._ : _Graphics).__softwareDirty)
		{
			...
			CanvasGraphics.graphics = graphics;
			CanvasGraphics.allowSmoothing = (renderer._ : _CanvasRenderer).__allowSmoothing;
			CanvasGraphics.worldAlpha = (renderer._ : _CanvasRenderer).__getAlpha(((graphics._ : _Graphics).__owner._ : _DisplayObject).__worldAlpha);
			bounds = (graphics._ : _Graphics).__bounds;
			...
			{
				(graphics._ : _Graphics).__renderData.canvas = null;
				(graphics._ : _Graphics).__renderData.context = null;
			...
			else
			{
				...
				{
					(graphics._ : _Graphics).__renderData.canvas = cast Browser.document.createElement("canvas");
					(graphics._ : _Graphics).__renderData.context = (graphics._ : _Graphics).__renderData.canvas.getContext("2d");
				}
				...
				(renderer._ : _CanvasRenderer).__setBlendModeContext(context, NORMAL);
				...
				for (type in (graphics._ : _Graphics).__commands.types)
				{
					switch (type)
					{
						...
						case OVERRIDE_BLEND_MODE:
							...
							(renderer._ : _CanvasRenderer).__setBlendModeContext(context, c.blendMode);
				...
				data.destroy();
				(graphics._ : _Graphics).__bitmap = BitmapData.fromCanvas((graphics._ : _Graphics).__renderData.canvas);
			}
