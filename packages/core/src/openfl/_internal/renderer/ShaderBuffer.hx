...
@:access(openfl.display._Shader) // TODO: Remove backend references
...
class ShaderBuffer
{
	...
	public function update(shader:GraphicsShader):Void
	{
		...
		{
			input = shader._.inputBitmapData[i];
		...
		var boolCount = shader._.paramBool.length;
		var floatCount = shader._.paramFloat.length;
		var intCount = shader._.paramInt.length;
