...
#if (cpp && windows)
import openfl.media.NativeVideo;
#end
...
class Video extends DisplayObject
{
	...
	**/
	...
	#if (cpp && windows)
	@:noCompletion private var __nativeVideo:NativeVideo;
	@:noCompletion private var __nativeVideoWidth:Int;
	@:noCompletion private var __nativeVideoHeight:Int;
	@:noCompletion private var __nativeClientMetaSent:Bool;
	@:noCompletion private var __nativeClientCompleteSent:Bool;
	@:noCompletion private var __nativeClientLastTime:Float;
	@:noCompletion private var __nativeWasPlaying:Bool;
	#end
	...
	**/
	public function new(width:Int = 320, height:Int = 240):Void
	{
		...
		#if (cpp && windows)
		__nativeClientMetaSent = false;
		__nativeClientCompleteSent = false;
		__nativeClientLastTime = -1;
		__nativeWasPlaying = false;
		#end
	}
	...
	**/
	public function attachNetStream(netStream:NetStream):Void
	{
		...
		if (__stream != null)
		{
			__stream.__nativeDetachVideo(this);
		}
		#end
		...
		if (__stream != null)
		{
			__stream.__nativeAttachVideo(this);
		}
		else
		{
			__netStreamClose();
	...
	**/
	...
	@:noCompletion private override function __enterFrame(deltaTime:Int):Void
	{
		...
		#elseif (cpp && windows)
		if (__renderable && __stream != null && __nativeVideo != null)
		{
			__nativeVideo.__updatePlaybackFrame();
			__stream.time = __nativeVideo.currentTime;
			__nativeDispatchMetaData();

			if (__nativeVideo.currentTime != __nativeClientLastTime)
			{
				__nativeClientLastTime = __nativeVideo.currentTime;
				__nativeDispatchPlayStatus("NetStream.Play.timeupdate");
			}

			var isNativePlaying = __nativeVideo.isPlaying;
			if (isNativePlaying)
			{
				__nativeWasPlaying = true;
			}

			if (!__nativeClientCompleteSent && __stream.__nativeIsPlaying && __nativeWasPlaying && !isNativePlaying)
			{
				__nativeClientCompleteSent = true;
				__nativeWasPlaying = false;
				__stream.__dispatchStatus("NetStream.Play.Stop");
				__stream.__dispatchStatus("NetStream.Play.Complete");
				__nativeDispatchPlayStatus("NetStream.Play.Complete");
			}
	...
	@:noCompletion private function __getTexture(context:Context3D):RectangleTexture
	{
		...
		return __texture;
		#elseif (cpp && windows)
		if (__nativeVideo == null || __nativeVideo.bitmapData == null) return null;

		var nativeTexture = __nativeVideo.bitmapData.getTexture(context);
		if (nativeTexture == null) return null;

		__texture = cast nativeTexture;
		__textureTime = __nativeVideo.currentTime;
		return __texture;
		#else
		return null;
		#end
	...
	@:noCompletion private function __getVertexBuffer(context:Context3D):VertexBuffer3D
	{
			...
			|| __currentHeight != height)
		{
			...
			__vertexBufferContext = context.__context;
			__vertexBuffer = context.createVertexBuffer(4, VERTEX_BUFFER_STRIDE);
	...
	#if (cpp && windows)
	@:noCompletion private function __nativeDispatchMetaData():Void
	{
		if (__stream == null || __nativeVideo == null || __nativeClientMetaSent)
		{
			return;
		}

		__nativeClientMetaSent = true;

		if (__stream.client != null)
		{
			try
			{
				var handler = __stream.client.onMetaData;
				handler({
					width: __nativeVideoWidth,
					height: __nativeVideoHeight,
					duration: __nativeVideo.duration
				});
			}
			catch (e:Dynamic) {}
		}
	...
	@:noCompletion private function __nativeDispatchPlayStatus(code:String):Void
	{
		if (__stream == null || __nativeVideo == null)
		{
			return;
		}
		...
		if (__stream.client != null)
		{
			try
			{
				var handler = __stream.client.onPlayStatus;
				handler({
					code: code,
					duration: __nativeVideo.duration,
					position: __nativeVideo.currentTime,
					speed: __stream.__nativeIsPlaying ? 1 : 0,
					start: 0
				});
			}
			catch (e:Dynamic) {}
	...
	@:noCompletion private function __netStreamPlay(source:String):Void
	{
		...
		if (__nativeVideo == null)
		{
			var initialWidth = __width > 0 ? Std.int(__width) : 0;
			var initialHeight = __height > 0 ? Std.int(__height) : 0;
			__nativeVideo = new NativeVideo(initialWidth, initialHeight, smoothing);
		}
		else
		{
			__nativeVideo.unload(true);
		}

		__nativeVideo.load(source);
		__nativeVideoWidth = __nativeVideo.__getVideoWidth();
		__nativeVideoHeight = __nativeVideo.__getVideoHeight();
		__nativeClientMetaSent = false;
		__nativeClientCompleteSent = false;
		__nativeClientLastTime = -1;
		__nativeWasPlaying = false;
		...
		if (__stream != null)
		{
			__stream.__dispatchStatus("NetStream.Play.Start");
		}
		__nativeDispatchPlayStatus("NetStream.Play.playing");
		__setRenderDirty();
	}

	@:noCompletion private function __netStreamPause():Void
	{
		if (__nativeVideo != null)
		{
			__nativeVideo.stop();
			__nativeWasPlaying = false;
			__nativeDispatchPlayStatus("NetStream.Play.pause");
		}
	}

	@:noCompletion private function __netStreamResume():Void
	{
		if (__nativeVideo != null)
		{
			__nativeVideo.play();
	...
	@:noCompletion private function __netStreamSeek(position:Float):Void
	{
		if (__nativeVideo != null)
		{
			__nativeVideo.currentTime = position;
			if (__stream != null)
			{
				__stream.__dispatchStatus("NetStream.Seek.Complete");
			}
			__nativeDispatchPlayStatus("NetStream.Play.seeking");
			__setRenderDirty();
		}
	}

	@:noCompletion private function __netStreamClose():Void
	{
		if (__nativeVideo != null)
		{
			__nativeVideo.unload(true);
		...
		__nativeVideoWidth = 0;
		__nativeVideoHeight = 0;
		__nativeClientMetaSent = false;
		__nativeClientCompleteSent = false;
		__nativeClientLastTime = -1;
		__nativeWasPlaying = false;
	...
	@:noCompletion private function __netStreamApplySoundTransform(value:SoundTransform):Void
	{
		if (__nativeVideo != null && value != null)
		{
			__nativeVideo.soundTransform = value;
		}
	}
	#end
