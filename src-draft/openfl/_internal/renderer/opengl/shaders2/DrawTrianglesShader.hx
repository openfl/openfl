...
class DrawTrianglesShader extends Shader
{
	public function new(context3D:Context3D)
	{
		...
		vertexSrc = [
			'attribute vec2 ${Attrib.Position};', 'attribute vec2 ${Attrib.TexCoord};', 'attribute vec4 ${Attrib.Color};',
			'uniform mat3 ${Uniform.ProjectionMatrix};', 'varying vec2 vTexCoord;', 'varying vec4 vColor;', 'void main(void) {',

			'   gl_Position = vec4((${Uniform.ProjectionMatrix} * vec3(${Attrib.Position}, 1.0)).xy, 0.0, 1.0);', '   vTexCoord = ${Attrib.TexCoord};',
		...
		fragmentSrc = [
			'#ifdef GL_ES', 'precision lowp float;', '#endif', 'uniform sampler2D ${Uniform.Sampler};', 'uniform vec3 ${Uniform.Color};',

			'uniform bool ${Uniform.UseTexture};', 'uniform float ${Uniform.Alpha};', 'uniform vec4 ${Uniform.ColorMultiplier};',
			'uniform vec4 ${Uniform.ColorOffset};', 'varying vec2 vTexCoord;', 'varying vec4 vColor;', 'vec4 tmp;',

			'vec4 colorTransform(const vec4 color, const vec4 tint, const vec4 multiplier, const vec4 offset) {',
			'   vec4 unmultiply = vec4(color.rgb / color.a, color.a);', '   vec4 result = unmultiply * tint * multiplier;', '   result = result + offset;',
			'   result = clamp(result, 0., 1.);', '   result = vec4(result.rgb * result.a, result.a);', '   return result;', '}', 'void main(void) {',

			'   if(${Uniform.UseTexture}) {', '       tmp = texture2D(${Uniform.Sampler}, vTexCoord);', '   } else {',
