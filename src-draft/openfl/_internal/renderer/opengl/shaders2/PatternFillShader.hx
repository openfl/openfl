...
class PatternFillShader extends Shader
{
	public function new(context3D:Context3D)
	{
		...
		fragmentSrc = [
			'#ifdef GL_ES', 'precision lowp float;', '#endif', 'uniform float ${Uniform.Alpha};', 'uniform vec2 ${Uniform.PatternTL};',

			'uniform vec2 ${Uniform.PatternBR};', 'uniform sampler2D ${Uniform.Sampler};', 'uniform vec4 ${Uniform.ColorMultiplier};',

			'uniform vec4 ${Uniform.ColorOffset};', 'varying vec2 vPosition;',

			'vec4 colorTransform(const vec4 color, const float alpha, const vec4 multiplier, const vec4 offset) {',
			'   vec4 unmultiply = vec4(color.rgb / color.a, color.a);', '   vec4 result = unmultiply * multiplier;', '   result.a *= alpha;',
			'   result = result + offset;', '   result = clamp(result, 0., 1.);', '   result = vec4(result.rgb * result.a, result.a);', '   return result;',
			'}', 'void main(void) {', '   vec2 pos = mix(${Uniform.PatternTL}, ${Uniform.PatternBR}, vPosition);',
