...
class PrimitiveShader extends Shader
{
	public function new(context3D:Context3D)
	{
		...
		vertexSrc = [
			'attribute vec2 ${Attrib.Position};', 'attribute vec4 ${Attrib.Color};', 'uniform mat3 ${Uniform.TranslationMatrix};',

			'uniform mat3 ${Uniform.ProjectionMatrix};', 'uniform vec4 ${Uniform.ColorMultiplier};', 'uniform vec4 ${Uniform.ColorOffset};',
			'uniform float ${Uniform.Alpha};', 'varying vec4 vColor;',

			'vec4 colorTransform(const vec4 color, const float alpha, const vec4 multiplier, const vec4 offset) {', '   vec4 result = color * multiplier;',
			'   result.a *= alpha;', '   result = result + offset;', '   result = clamp(result, 0., 1.);',
			'   result = vec4(result.rgb * result.a, result.a);', '   return result;', '}', 'void main(void) {',
