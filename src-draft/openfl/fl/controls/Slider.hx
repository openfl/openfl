...
class Slider extends UIComponent
{
	...
	private function setState(state:String)
	{
		...
		getXFLMovieClip("SliderThumb_" + this.state + "Skin").x = getXFLMovieClip("SliderTrack_skin").x +
			(sliderRange < 0.00001 ? 0.0 : ((value - minimum) / sliderRange) * getXFLMovieClip("SliderTrack_skin").width);
